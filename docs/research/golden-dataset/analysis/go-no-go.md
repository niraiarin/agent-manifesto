...
**最終バッチ実験結果** (qwen3.6-35b-a3b-bf16 + FP16 混在, 26件入力):
- BF16 (新): 11件 (001-004, 012-018) + 7件 (005-011) = 18件
- FP16 (元): 8件 (019-026)

| 指標 | 値 | 前回 (FP16 only) |
|------|----|-----------------|
| 実行成功率 | 26/26 (100%) — エラー 0 件 | 26/26 (100%) |
| avg_delta | **0.046** | 0.069 |
| pass_rate (|Δ| ≤ 0.5, 対称) | **20/26 (77%)** | 18/26 (69%) |
| pass_rate (Δ ≤ 0.5, **非対称**) | **23/26 (88%)** | 23/26 (88%) |
| pass_rate (|Δ| ≤ 0.6, 緩和) | **26/26 (100%)** | 26/26 (100%) |
| Local > Cloud | **7件** (Δ=-0.2 〜 -0.6) | 5件 (Δ=-0.6) |

FAIL 6件は全て |Δ|=0.6 で閾値を 0.1 超過のみ。うち 3件は Local > Cloud。

**閾値定義の補足**: 本研究の目的は「Local が Cloud 相当以上」の判定のため、
非対称閾値 `Δ ≤ 0.5` (Local が Cloud より 0.5 以内しか劣化しない) が妥当。
Local > Cloud (Δ<0) は本来 PASS 扱いすべきで、その基準では pass=23/26 (88%)。

判定スコア粒度が 0.2 刻み (1-5 スケール × 5 基準の平均) のため、
|Δ|=0.6 は実質最小有意差 3 刻み分。ノイズマージン考慮で `|Δ|≤0.6` なら pass=26/26 (100%)。

**BF16 の効果**: 新規 BF16 7件 (005-011) は全件 pass、うち 4件が Local > Cloud で、
BF16 が Cloud 相当以上の品質を安定して出せることを確認。
...
- T-interp: 26件入力で avg_delta=0.046, pass_rate=88% (非対称閾値) / 100% (|Δ|≤0.6)
- 統計検定 (Brunner-Munzel) を推奨するが、効果量は一貫

### 品質 Gate: **PASS**
- T-interp: **PASS** — avg_delta=0.046、error_rate=0%、非対称 pass=88%
