...
| 2026-04-18 | **T-interp BF16 部分再実行 (18/26)**: 001-004, 005-018 の計 18件を BF16 (ctx=131072) で再実行、全件 OK。ゾンビバッチ発生 (pkill で parent script が殺せず 3並行稼働) により副産物的に 11件が並行処理されたが、整理後に FAIL 残 7件を rerun-t-interp-fails.sh で修復。全 26 評価: **avg_delta=0.046 (FP16 0.069 から改善), pass=20/26 (77%, 対称) → 23/26 (88%, 非対称) → 26/26 (100%, |Δ|≤0.6)**。新 BF16 7件で 4件 Local>Cloud を確認。閾値定義の議論: 非対称 `Δ≤0.5` が目的関数に適合 |
| 2026-04-19 | **BF16 統一データセット完成 (55件全 BF16)**: M-interp 残外れ値 2件 (019, 022) + T-interp FP16 残 8件 (019-026) を BF16 で再実行。全 10件 OK。最終結果 — M-interp 29/29: avg_delta=0.290, pass=26/29 (90%, 対称), 29/29 (100%, |Δ|≤0.6), **外れ値解消** (旧 Δ=2.0/3.6 → 0.2/0.4)。T-interp 26/26: avg_delta=0.096 (外れ値除), pass=20/25 (80%, 対称), 21/25 (84%, 非対称), 25/25 (100%, |Δ|≤0.6 外れ値除)。T-interp-023 (Δ=+1.6) が新外れ値 (出力構造は正常、judge 差分)。**Sub-3 完全クローズ: GO** |
| 2026-04-19 | **Sub-4: RouteLLM 変換データ生成**: 55件の judge 評価結果を Chatbot Arena battle format (jsonl) に変換。`convert-to-routellm.py` スクリプト + 2 閾値版 (0.5 / 0.3) + README を `routellm/` 配下に配置。Winner 分布 (th=0.5): model_a (cloud)=8 (15%), model_b (local)=1 (2%), tie=46 (84%)。Threshold=0.3 で tie=49% に減少、学習信号強化。制限: サンプル数少 → Arena との混合 fine-tuning が現実的 |
