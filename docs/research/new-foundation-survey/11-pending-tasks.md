...
| **Day 11+** | **02-data-provenance §4.4-4.7 完全実装** + **PROV-O relation Lean 化** (Day 10 評価から、wasAttributedTo / wasGeneratedBy / wasDerivedFrom) + **Day 6-10 評価から繰り延ばし項目**: 🟡 02-data-provenance §4.4 退役の構造的検出 (RetiredEntity) / 🟡 PROV-O relation の Lean 化 (Day 10 から) / 🟡 §4.6 Nextflow resume + Galaxy job cache / 🟡 Hypothesis rationale 型化 / 🟡 Failure payload 型化 / 🟡 Verdict payload 拡充 / 🟢 transitionLegacy deprecated 削除 / 🟢 EvolutionStep 完全 4 member 化 (G5-1 §3.4 step 1) / 🟢 G5-1 §3.4 step 2 LearningM indexed monad (連携 path 完備で本格実装可能) / 🟢 S6 Paper 1 BST/AVL invariants / 🟢 Evolution DecidableEq / 🟢 HandoffChain concat / 🟢 ResearchEntity DecidableEq 手動実装 / 🟢 ResearchActivity payload なし variants の payload 拡充 / 🟢 HandoffChain 全体 embed 用 constructor。または **Manifest 移植** (Week 3-4) または **Lean-Auto research/PoC** (Section 2.10 🔴) を並行開始 | Week 3-4 (Manifest) / Week 4-5 (PROV-O relation + LearningM) / Week 6 (Lean-Auto) / Section 2.10 + 2.12 + 2.13 + 2.15 + 2.17 + 2.19 (Day 6-10 評価導出) |
