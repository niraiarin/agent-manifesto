...
| **Day 16** | Day 15 議論で確定済 Q1-Q4 (Section 2.30): **Q1 B 案** = transitionLegacy 削除 (Day 14 `@[deprecated]` + Day 15 `@[retired]` 両モデル転用、cycle 内学習 transfer 2 段階別分野転用実例、Section 2.15 Day 9+ 繰り延べ課題解消) + **Q2 A-Compact** = `@[deprecated]` 付与 + 利用箇所移行 + 定義残置 (backward compatibility 維持、完全削除は Day 17+) + **Q3 案 A** = `AgentSpec/Spine/EvolutionStep.lean` MODIFY (transitionLegacy deprecated 付与 + TransitionReflexive/Transitive 新 signature 移行) + **Q4 案 A** = `AgentSpec/Test/Spine/EvolutionStepTest.lean` MODIFY (既存 test 更新 + rfl preference 維持、cycle 内学習 transfer 5 度目、Day 11-16 = 6 Day 連続記録更新): EvolutionStep.lean (MODIFY) + EvolutionStepTest.lean (MODIFY、+3-5 example) + artifact-manifest entry 更新 + deprecation_history field 新規追加 + version `0.16.0-week2-day16` | Section 2.30 (Day 16 着手前判断) + Section 12.43-12.45 (Day 15 評価導出) + Section 2.15 (Day 8 evaluator 繰り延べ課題解消) + TyDD-G3 + cycle 内学習 transfer 別分野転用 |
| **Day 17+** | **Day 15 評価で繰り延べ + 新規識別項目**: 🟡 ResearchActivity payload 拡充 (Day 13-15 から継続) / 🟢 ResearchEntity DecidableEq 手動実装 / 🟢 EvolutionStep 完全 4 member 化 / 🟢 G5-1 §3.4 step 2 LearningM indexed monad / 🟢 A-Maximal elaborator 型レベル強制 (Week 5-6 Tooling 層本丸) / 🟢 Day 14 + Day 15 両モデル cross-interaction test 拡張 / 🟢 WasDerivedFrom DAG 制約 (Subagent I2 Day 11 設計判断、Section 2.21/2.23/2.25/2.27/2.29 から継続) + **Day 6-14 評価から繰り延べ項目**: 🟡 §4.6 Nextflow resume + Galaxy job cache / 🟡 Hypothesis rationale 型化 / 🟡 Failure payload 型化 / 🟡 Verdict payload 拡充 / 🟢 S6 Paper 1 BST/AVL invariants / 🟢 Evolution DecidableEq / 🟢 HandoffChain concat / 🟢 HandoffChain 全体 embed 用 constructor。または **Manifest 移植** (Week 3-4) または **Lean-Auto research/PoC** (Section 2.10 🔴) を並行開始 | Week 3-4 (Manifest) / Week 4-5 (LearningM) / Week 6 (Lean-Auto) / Section 2.10 + 2.12 + 2.13 + 2.15 + 2.17 + 2.19 + 2.21 + 2.23 + 2.25 + 2.27 + 2.29 |
