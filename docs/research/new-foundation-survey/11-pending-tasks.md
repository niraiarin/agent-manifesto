...
| **Day 10** | Day 9 議論で確定済 Q1-Q4 (Section 2.18): **Q1 B 案** = ResearchAgent + EvolutionStep transition → ResearchActivity.verify mapping (Day 8/9 連携 path 確立) + **Q2 B-Medium** = signature + 実装 + test + **Q3 案 A** = `structure ResearchAgent { identity, role : Role }` + `inductive Role { Researcher, Reviewer, Verifier }` + **Q4 案 A** = free function `transitionToActivity (h : Hypothesis) (v : Verdict) : ResearchActivity := .verify h v`: `AgentSpec/Provenance/{ResearchAgent, EvolutionMapping}.lean` + ResearchEntity.lean 5 constructor 拡張検討 (Agent embed の設計判断は実装時) + `AgentSpec/Test/Provenance/{ResearchAgentTest, EvolutionMappingTest}.lean` | Section 2.18 (Day 10 着手前判断) + Section 12.25-12.27 (Day 9 評価導出) + 02-data-provenance §4.1 (PROV-O Agent + Entity/Activity 統合) |
| **Day 11+** | **02-data-provenance §4.4-4.7 完全実装** + **Day 6-9 評価から繰り延べ項目** (Day 10 で扱わなかったもの): 🟡 02-data-provenance §4.4 退役の構造的検出 / 🟡 §4.6 Nextflow resume + Galaxy job cache / 🟡 Hypothesis rationale 型化 / 🟡 Failure payload 型化 / 🟡 Verdict payload 拡充 / 🟢 transitionLegacy deprecated 削除 / 🟢 EvolutionStep 完全 4 member 化 / 🟢 G5-1 §3.4 step 2 LearningM indexed monad / 🟢 S6 Paper 1 BST/AVL invariants / 🟢 Evolution DecidableEq / 🟢 HandoffChain concat / 🟢 ResearchEntity DecidableEq 手動実装 / 🟢 ResearchActivity payload なし variants の payload 拡充 / 🟢 HandoffChain 全体 embed 用 constructor。または **Manifest 移植** (Week 3-4) または **Lean-Auto research/PoC** (Section 2.10 🔴) を並行開始 | Week 3-4 (Manifest) / Week 4-5 (PROV-O 完全実装 + LearningM) / Week 6 (Lean-Auto) / Section 2.10 + 2.12 + 2.13 + 2.15 + 2.17 |
