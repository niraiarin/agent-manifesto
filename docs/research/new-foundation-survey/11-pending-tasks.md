...
| **Day 10+** | **Provenance 層完成** (ResearchAgent + 02-data-provenance §4.4-4.7 完全実装) + **Day 6-9 評価から繰り延げ項目**: 🟡 ResearchAgent (Day 9 A-Minimal で繰り延べ、Day 10 メイン候補) / 🟡 EvolutionStep transition → ResearchActivity.verify mapping (Day 9 Section 2.17 から、verify ≡ B4 整合の連携 path 実装) / 🟡 02-data-provenance §4.4 退役の構造的検出 / 🟡 §4.6 Nextflow resume + Galaxy job cache / 🟡 Hypothesis rationale 型化 / 🟡 Failure payload 型化 / 🟡 Verdict payload 拡充 / 🟢 transitionLegacy deprecated 削除 / 🟢 EvolutionStep 完全 4 member 化 / 🟢 G5-1 §3.4 step 2 LearningM indexed monad / 🟢 S6 Paper 1 BST/AVL invariants / 🟢 Evolution DecidableEq / 🟢 HandoffChain concat / 🟢 ResearchEntity DecidableEq 手動実装 / 🟢 ResearchActivity payload なし variants の payload 拡充 / 🟢 HandoffChain 全体 embed 用 constructor (Day 10+ 設計判断、Subagent I3 Day 9)。または **Manifest 移植** (Week 3-4) または **Lean-Auto research/PoC** (Section 2.10 🔴) を並行開始 | Week 3-4 (Manifest) / Week 4-5 (PROV-O 完成 + LearningM) / Week 6 (Lean-Auto) / Section 2.10 + 2.12 + 2.13 + 2.15 + 2.17 (Day 6-9 評価導出) |
