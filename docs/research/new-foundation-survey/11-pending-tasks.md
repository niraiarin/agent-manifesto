...
| **Day 15** | Day 14 議論で確定済 Q1-Q4 (Section 2.28): **Q1 A 案** = A-Compact custom attribute `@[retired]` (Day 14 A-Minimal の段階的拡張第 2 段階) + **Q2 A-Compact-Hybrid** = Lean 4 elab macro で `@[retired msg since]` を `@[deprecated msg (since := since)]` に展開 (Day 14 backward compatible) + **Q3 案 B** = 新 module `AgentSpec/Provenance/RetirementLinter.lean` (NEW) に macro 定義のみ (RetiredEntity.lean は変更なし) + **Q4 案 A** = 新 file `AgentSpec/Test/Provenance/RetirementLinterTest.lean` (NEW) で macro 動作確認 + rfl preference 維持 (cycle 内学習 transfer 4 度目、Day 11-15 = 5 Day 連続): `AgentSpec/Provenance/RetirementLinter.lean` (NEW、macro_rules 定義) + `AgentSpec/Test/Provenance/RetirementLinterTest.lean` (NEW、推定 8-12 example) + linter_status A-Minimal → A-Compact (Hybrid macro) 拡張 + version `0.15.0-week2-day15` | Section 2.28 (Day 15 着手前判断) + Section 12.40-12.42 (Day 14 評価導出) + TyDD-G3 + 02-data-provenance §4.4 |
| **Day 16+** | **Day 14 評価で繰り延べ + 新規識別項目**: 🟡 ResearchActivity payload 拡充 (Day 13-14 から継続) / 🟢 A-Standard custom linter (Lean.Elab.Command 拡張) / 🟢 ResearchEntity DecidableEq 手動実装 (Day 12-14 で全 type 制約継承継続) / 🟢 transitionLegacy deprecated 削除 (Day 14 `@[deprecated]` モデル転用可能、cycle 内学習 transfer 別分野拡張) / 🟢 EvolutionStep 完全 4 member 化 / 🟢 G5-1 §3.4 step 2 LearningM indexed monad / 🟢 A-Maximal elaborator 型レベル強制 (Week 5-6 Tooling 層本丸) / 🟢 WasDerivedFrom DAG 制約 (Subagent I2 Day 11 設計判断、Section 2.21/2.23/2.25/2.27 から継続) + **Day 6-13 評価から繰り延べ項目**: 🟡 §4.6 Nextflow resume + Galaxy job cache / 🟡 Hypothesis rationale 型化 / 🟡 Failure payload 型化 / 🟡 Verdict payload 拡充 / 🟢 S6 Paper 1 BST/AVL invariants / 🟢 Evolution DecidableEq / 🟢 HandoffChain concat / 🟢 HandoffChain 全体 embed 用 constructor。または **Manifest 移植** (Week 3-4) または **Lean-Auto research/PoC** (Section 2.10 🔴) を並行開始 | Week 3-4 (Manifest) / Week 4-5 (LearningM) / Week 6 (Lean-Auto) / Section 2.10 + 2.12 + 2.13 + 2.15 + 2.17 + 2.19 + 2.21 + 2.23 + 2.25 + 2.27 |
