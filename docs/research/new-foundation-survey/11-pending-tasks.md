...
| **Day 15** | **Day 14 議論で確定するメイン候補** (Day 14 評価 Section 12.40 / 12.41 / 2.27 から導出): 🟡 A-Compact custom attribute `@[retired]` (Day 14 A-Minimal の自然な拡張、Lean 4 `register_simp_attr` + elaborator hook) ほか副候補は Day 14 完了後の議論 (Section 2.28 で確定予定) | Section 2.27 (Day 14 評価) + Section 12.40-12.42 + TyDD-G3 + 02-data-provenance §4.4 |
