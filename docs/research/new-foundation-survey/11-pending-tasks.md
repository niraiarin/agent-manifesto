...
- **Week 1 からの持ち越し優先タスク** (Section 2 から、Day 2 終了時点の状態):
  ...
  - 🔄 継続中 — Top-down / hole-driven 実装スタイル採用 (Section 2.6、Day 1-2 で実践、Day 3-5 で継続)
  - ✅ Day 1-2 進行 — FolgeID (GA-S2) Day 1 完了 / Edge Type (GA-S4) Day 2 完了。FolgeID PartialOrder 拡張は Day 5、Edge dependent type 化は Week 4-5 (Section 2.8)
...
| **Day 4** | Spine 層 `LearningCycle.lean`, `Observable.lean` の type class 宣言 + dummy instance (**Day 1 確立パターン適用**) **+ Day 2 評価から**: F2 Lattice (LearningCycle 収束) / H7 minimal viable pipeline / S4 P5 explicit assumptions | Week 2-3 Spine 層 / Section 10.2 / 12.6 |
