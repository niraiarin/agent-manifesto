...
- **Week 1 からの持ち越し優先タスク** (Section 2 から、Day 9 終了時点の状態):
  ...
  - 🔄 継続中 — Top-down / hole-driven 実装スタイル採用 (Section 2.6、Day 1-9 で実践)
  - ✅ **Day 1-5 Spine 層完了 + Day 6-7 Process 層完備 + Day 8 Section 2.9 完全解消 + Day 9 Provenance 層 3 type 完備** — FolgeID (GA-S2) Day 1 / Edge (GA-S4) Day 2 / EvolutionStep + SafetyConstraint (GA-S1) Day 3 / LearningCycle + Observable (GA-S1) Day 4 で **Section 1 Week 2-3 完了基準 (4 type class + dummy instance) 達成**。**Day 5 で順序関係完備**。**Day 6-7 で Process 層 4 type 完備**。**Day 8 で Section 2.9 完全解消** (Verdict + EvolutionStep B4 4-arg post)。**Day 9 で Provenance 層 3 type 完備** (Verdict + ResearchEntity + ResearchActivity、ResearchAgent のみ Day 10+)。Edge dependent type 化は Week 4-5 (Section 2.8)、SafetyConstraint Bool→Prop は Day 4 完了 (Section 2.9 🔴 解消)、**Pattern #7 hook 化は Day 5 構造的解決 + Day 6/7/8/9 で 4 度連続運用検証成功** (Section 6.2.1 完全 closure + 運用安定性確立)。**Day 9 で namespace extension pattern 確立** (循環依存回避設計)。Ord (lex total order) / S6 BST/AVL invariants / ResearchAgent / EvolutionStep transition → ResearchActivity.verify mapping / DecidableEq / payload 拡充 は Day 10+ へ繰り延げ
...
| 🟡 Day 10+ | **EvolutionStep transition → ResearchActivity.verify mapping** — Day 8 B4 4-arg post と Day 9 ResearchActivity.verify (input : Hypothesis, output : Verdict) 整合の連携 path 実装 | G5-1 §3.4 step 2 / 02-data-provenance §4.1 | Day 10+ Provenance 層完成時 (Day 9 paper サーベイ評価で識別) |
...
| **Day 9** | Day 8 議論で確定済 Q1-Q4 (Section 2.16): **Q1 A 案** = Provenance 層継続 + **Q2 A-Minimal** = ResearchEntity + ResearchActivity 2 type + Mapping + **Q3 案 A** = ResearchEntity 4 constructor (既存 Process type embed) + **Q4 案 A** = Process side `.toEntity` method (実装は循環依存回避のため ResearchEntity.lean 内に配置): `AgentSpec/Provenance/{ResearchEntity, ResearchActivity}.lean` + 4 Process .lean modify (toEntity 追加) + `AgentSpec/Test/Provenance/{ResearchEntityTest, ResearchActivityTest}.lean` | Section 2.16 (Day 9 着手前判断) + Section 12.22-12.24 (Day 8 評価導出) + 02-data-provenance §4.1 (PROV-O 実装) |
| **Day 10+** | **Provenance 層完成** (ResearchAgent + 02-data-provenance §4.4-4.7 完全実装) + **Day 6-8 評価から繰り延げ項目**: 🟡 ResearchAgent (Day 9 A-Minimal で繰り延げ) / 🟡 02-data-provenance §4.4 退役の構造的検出 / 🟡 §4.6 Nextflow resume + Galaxy job cache / 🟡 Hypothesis rationale 型化 / 🟡 Failure payload 型化 / 🟡 Verdict payload 拡充 (案 A→C 移行) / 🟢 transitionLegacy deprecated 削除 / 🟢 EvolutionStep 完全 4 member 化 / 🟢 G5-1 §3.4 step 2 LearningM indexed monad / 🟢 S6 Paper 1 BST/AVL invariants / 🟢 Evolution DecidableEq / 🟢 HandoffChain concat。または **Manifest 移植** (Week 3-4) または **Lean-Auto research/PoC** (Section 2.10 🔴) を並行開始 | Week 3-4 (Manifest) / Week 4-5 (PROV-O 完成 + LearningM) / Week 6 (Lean-Auto) / Section 2.10 + 2.12 + 2.13 + 2.15 |
