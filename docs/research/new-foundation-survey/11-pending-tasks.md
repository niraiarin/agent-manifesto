...
- **Week 1 からの持ち越し優先タスク** (Section 2 から、Day 10 終了時点の状態):
  ...
  - ✅ **Day 1-5 Spine 層完了 + Day 6-7 Process 層完備 + Day 8 Section 2.9 完全解消 + Day 9 Provenance 層 3 type + Day 10 Provenance 4 type 完備 (PROV-O 三項統合完了) + Day 11 Provenance 4 type + 3 relation 完備 (PROV-O §4.1 完全カバー到達) + Day 12 Provenance 5 type + 3 relation 完備 (PROV-O §4.1 + §4.4 同時完全カバー到達) + Day 13 Provenance 5 type + 6 relation 完備 (PROV-O §4.1 main + auxiliary + §4.4 完全カバー)** — FolgeID (GA-S2) Day 1 / Edge (GA-S4) Day 2 / EvolutionStep + SafetyConstraint (GA-S1) Day 3 / LearningCycle + Observable (GA-S1) Day 4 で **Section 1 Week 2-3 完了基準 (4 type class + dummy instance) 達成**。**Day 5 で順序関係完備**。**Day 6-7 で Process 層 4 type 完備**。**Day 8 で Section 2.9 完全解消**。**Day 9 で Provenance 層 3 type**、**Day 10 で Provenance 層 4 type 完備 (PROV-O 三項統合完了 + EvolutionMapping 連携 path 確立)**、**Day 11 で PROV-O 3 main relation 完備 (WasAttributedTo + WasGeneratedBy + WasDerivedFrom = §4.1 完全カバー到達)**、**Day 12 で RetiredEntity + RetirementReason 4 variant 完備 (§4.4 完全カバー到達 = §4.1 + §4.4 同時完全カバー)**、**Day 13 で PROV-O auxiliary relations (WasInformedBy + ActedOnBehalfOf) + WasRetiredBy 3 structure 完備 (§4.1 main + auxiliary + §4.4 relation 主要 spec 統合 = 6 relation 完備到達)**。**Pattern #7 hook 化は Day 5 構造的解決 + Day 6/7/8/9/10 で 5 度連続運用検証成功 + Day 10 v2 拡張 + Day 11 v2 初運用検証 + Day 12 v2 2 度目運用検証 + Day 13 v2 3 度目運用検証 = 運用定常化** (Section 6.2.1 完全 closure + 運用安定性確立 + governance evolution + 拡張運用検証 3 度連続、Provenance/Test/Cross detection 動作確認)。**Day 10 で layer architecture 完成形** (Spine + Process + Provenance + Cross test の 4 layer)。Ord (lex total order) / S6 BST/AVL invariants / DecidableEq 手動実装 / payload 拡充 / transitionLegacy 削除 / EvolutionStep 4 member 化 / LearningM / RetiredEntity linter / ResearchActivity payload 拡充 は Day 14+ へ繰り延げ
...
| **Day 14** | Day 13 議論で確定済 Q1-Q4 (Section 2.26): **Q1 A 案** = RetiredEntity linter / elaborator (新分野、Day 12-13 構造完備の強制化) + **Q2 A-Minimal** = Lean 4 標準 `@[deprecated]` 付与 + Test (1 日完結、Day 15+ で段階的拡張) + **Q3 案 A** = `@[deprecated "退役済 entity - RetirementReason を確認"]` を `RetiredEntity.trivial` 等 test fixture に手動付与 (production code 変更なし、backward compatible) + **Q4 案 C** = production code docstring で使用例注記 + Test は deprecated fixture の Inhabited / mk' rfl 確認のみ (cycle 内学習 transfer 3 度目適用、Day 11-14 で 4 Day 連続 rfl preference 維持): `AgentSpec/Provenance/RetiredEntity.lean` (MODIFY、deprecated fixture 追加 + docstring) + `AgentSpec/Test/Provenance/RetiredEntityTest.lean` (MODIFY、deprecated fixture 動作確認 6-10 example 追加) + custom attribute / linter / elaborator は Day 15+ で段階的拡張 (A-Compact → A-Standard → A-Maximal) | Section 2.26 (Day 14 着手前判断) + Section 12.37-12.39 (Day 13 評価導出) + 02-data-provenance §4.4 (退役 entity の構造的検出 強制化) |
| **Day 15+** | **Day 13 評価で繰り延べ + 新規識別項目**: 🟡 ResearchActivity payload 拡充 (Day 13 WasRetiredBy 案 C 考察分の繰り延べ、verify variant 同パターン) / 🟢 ResearchEntity DecidableEq 手動実装 (Day 13 で全 type 制約継承継続) / 🟢 transitionLegacy deprecated 削除 / 🟢 EvolutionStep 完全 4 member 化 / 🟢 G5-1 §3.4 step 2 LearningM indexed monad / 🟢 WasDerivedFrom DAG 制約 (Subagent I2 Day 11 設計判断、Section 2.21/2.23/2.25 から継続) + **Day 6-12 評価から繰り延べ項目**: 🟡 §4.6 Nextflow resume + Galaxy job cache / 🟡 Hypothesis rationale 型化 / 🟡 Failure payload 型化 / 🟡 Verdict payload 拡充 / 🟢 S6 Paper 1 BST/AVL invariants / 🟢 Evolution DecidableEq / 🟢 HandoffChain concat / 🟢 HandoffChain 全体 embed 用 constructor。または **Manifest 移植** (Week 3-4) または **Lean-Auto research/PoC** (Section 2.10 🔴) を並行開始 | Week 3-4 (Manifest) / Week 4-5 (LearningM) / Week 6 (Lean-Auto) / Section 2.10 + 2.12 + 2.13 + 2.15 + 2.17 + 2.19 + 2.21 + 2.23 + 2.25 |
