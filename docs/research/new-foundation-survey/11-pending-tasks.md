...
- **Week 1 からの持ち越し優先タスク** (Section 2 から、Day 10 終了時点の状態):
  ...
  - ✅ **Day 1-5 Spine 層完了 + Day 6-7 Process 層完備 + Day 8 Section 2.9 完全解消 + Day 9 Provenance 層 3 type + Day 10 Provenance 4 type 完備 (PROV-O 三項統合完了) + Day 11 Provenance 4 type + 3 relation 完備 (PROV-O §4.1 完全カバー到達)** — FolgeID (GA-S2) Day 1 / Edge (GA-S4) Day 2 / EvolutionStep + SafetyConstraint (GA-S1) Day 3 / LearningCycle + Observable (GA-S1) Day 4 で **Section 1 Week 2-3 完了基準 (4 type class + dummy instance) 達成**。**Day 5 で順序関係完備**。**Day 6-7 で Process 層 4 type 完備**。**Day 8 で Section 2.9 完全解消**。**Day 9 で Provenance 層 3 type**、**Day 10 で Provenance 層 4 type 完備 (PROV-O 三項統合完了 + EvolutionMapping 連携 path 確立)**、**Day 11 で PROV-O 3 main relation 完備 (WasAttributedTo + WasGeneratedBy + WasDerivedFrom = §4.1 完全カバー到達)**。**Pattern #7 hook 化は Day 5 構造的解決 + Day 6/7/8/9/10 で 5 度連続運用検証成功 + Day 10 v2 拡張 + Day 11 v2 初運用検証** (Section 6.2.1 完全 closure + 運用安定性確立 + governance evolution + 拡張運用検証、Provenance/Test/Cross detection 動作確認)。**Day 10 で layer architecture 完成形** (Spine + Process + Provenance + Cross test の 4 layer)。Ord (lex total order) / S6 BST/AVL invariants / DecidableEq 手動実装 / payload 拡充 / transitionLegacy 削除 / EvolutionStep 4 member 化 / LearningM / RetiredEntity / PROV-O auxiliary relations (WasInformedBy / ActedOnBehalfOf) は Day 12+ へ繰り延げ
...
| **Day 12** | Day 11 議論で確定済 Q1-Q4 (Section 2.22): **Q1 A 案** = RetiredEntity (02-data-provenance §4.4 完全カバー) + **Q2 A-Minimal** = `structure RetiredEntity` + `inductive RetirementReason` + Test + **Q3 案 A** = RetirementReason 4 variant 型化 (`Refuted (failure : Failure)` / `Superseded (replacement : ResearchEntity)` / `Obsolete` / `Withdrawn`、Failure 経由も payload で両立) + **Q4 案 A** = separate structure 配置 (Day 11 ProvRelation パターン踏襲、ResearchEntity 拡張不要 backward compatible): `AgentSpec/Provenance/RetiredEntity.lean` (NEW) + `AgentSpec/Test/Provenance/RetiredEntityTest.lean` (NEW、推定 18-25 example) + linter / elaborator は Day 13+ で別途集中 (新分野) | Section 2.22 (Day 12 着手前判断) + Section 12.31-12.33 (Day 11 評価導出) + 02-data-provenance §4.4 (退役 entity の構造的検出) |
| **Day 13+** | **Day 11 評価で繰り延べ + 新規識別項目**: 🟢 PROV-O auxiliary relations (`WasInformedBy` Activity → Activity / `ActedOnBehalfOf` Agent → Agent、Day 11 paper サーベイで新規識別) / 🟢 WasDerivedFrom DAG 制約 (Subagent I2 Day 11 設計判断、self-derivation 禁止) / 🟢 Test 内 simp tactic vs rfl preference (Subagent I3 Day 11 で識別) + **Day 6-10 評価から繰り延べ項目**: 🟡 §4.6 Nextflow resume + Galaxy job cache / 🟡 Hypothesis rationale 型化 / 🟡 Failure payload 型化 / 🟡 Verdict payload 拡充 / 🟢 transitionLegacy deprecated 削除 / 🟢 EvolutionStep 完全 4 member 化 / 🟢 G5-1 §3.4 step 2 LearningM indexed monad / 🟢 S6 Paper 1 BST/AVL invariants / 🟢 Evolution DecidableEq / 🟢 HandoffChain concat / 🟢 ResearchEntity DecidableEq 手動実装 / 🟢 ResearchActivity payload なし variants の payload 拡充 / 🟢 HandoffChain 全体 embed 用 constructor。または **Manifest 移植** (Week 3-4) または **Lean-Auto research/PoC** (Section 2.10 🔴) を並行開始 | Week 3-4 (Manifest) / Week 4-5 (LearningM) / Week 6 (Lean-Auto) / Section 2.10 + 2.12 + 2.13 + 2.15 + 2.17 + 2.19 + 2.21 |
