...
- **Week 1 からの持ち越し優先タスク** (Section 2 から、Day 6 終了時点の状態):
  ...
  - 🔄 継続中 — Top-down / hole-driven 実装スタイル採用 (Section 2.6、Day 1-6 で実践)
  - ✅ **Day 1-5 Spine 層完了 + Day 6 Process 層着手** — FolgeID (GA-S2) Day 1 / Edge (GA-S4) Day 2 / EvolutionStep + SafetyConstraint (GA-S1) Day 3 / LearningCycle + Observable (GA-S1) Day 4 で **Section 1 Week 2-3 完了基準 (4 type class + dummy instance) 達成**。**Day 5 で順序関係完備** (LearningStage LE/LT + FolgeID PartialOrder/LT)。**Day 6 で Process 層着手** (Hypothesis + Failure 2 inductive、02-data-provenance §4.3 100% 忠実実装、Pattern #7 hook 運用検証完了)。Edge dependent type 化は Week 4-5 (Section 2.8)、SafetyConstraint Bool→Prop は Day 4 で前倒し完了 (Section 2.9 🔴 解消)、**Pattern #7 hook 化は Day 5 で構造的解決 + Day 6 で運用検証完了** (Section 6.2.1 完全 closure)。Ord (lex total order) は Day 7+/Week 4-5 へ繰り延げ。Process 層継続 (Evolution + HandoffChain) は Day 7、Provenance 層 (PROV-O Lean 化) は Day 8+
...
#### Day 7 想定 (Day 6 評価 Section 12.17 / 2.12 改善余地反映、確定版)

| ファイル | 内容 |
|---|---|
| `AgentSpec/Process/Evolution.lean` | `inductive Evolution` (Hypothesis 依存、step transition + **EvolutionStep B4 4-arg post 統合**、Section 2.9 残課題解消)。Hypothesis chain order を **S6 Paper 1 BST/AVL invariants** で型レベル強制候補 (Section 2.10 paper-grounded) |
| `AgentSpec/Process/HandoffChain.lean` | `inductive HandoffChain` (T1 一時性、handoff sequence 表現)。Day 6 LearningCycle/Observable と組合わせて handoff state machine 構築 |
| `AgentSpec/Test/Process/EvolutionTest.lean` | Evolution 単独 test + **cross-process interaction test** (Hypothesis × Failure × Evolution、Day 4 fullSpineExample パターン踏襲、Section 2.12 🟡 解消) |
| `AgentSpec/Test/Process/HandoffChainTest.lean` | HandoffChain 単独 test + LearningCycle/Observable 統合 test |
...
| 優先度 | 項目 | 反映先 |
|---|---|---|
| 🟡 中 | **cross-process interaction test** (Hypothesis × Failure × Evolution) | Evolution Test ファイル (Section 2.12 🟡 解消予定) |
| 🟡 中 | **EvolutionStep B4 4-arg post の I/O type 確定** (Hypothesis/Verdict/Observable 統合) | Evolution.lean の signature (Section 2.9 残課題解消予定) |
| 🟡 中 | **HandoffChain の T1 一時性 inductive 表現** | HandoffChain.lean (Section 2.11 Day 7 既定義) |
| 🟢 低 | **S6 Paper 1 BST/AVL invariants** — Hypothesis chain の order を invariant 付き structure 化 | Evolution.lean (Section 2.10 paper-grounded) |

#### Day 8+ 想定 (Provenance 層着手、Day 6 評価 Section 12.16 / 2.10 改善反映)
...
| `AgentSpec/Provenance/Mapping.lean` | `Hypothesis.toEntity` / `Failure.toEntity` / `Evolution.toActivity` 等 mapping 関数。**Hypothesis rationale Refinement 強化** (Option String → Option Evidence、Section 2.12 🟡 解消) と **Failure payload 型化** (各 variant payload を専用型に、Section 2.12 🟡 解消) を同時実施 |
...
| **Day 7** | Process 層継続: `AgentSpec/Process/Evolution.lean` (Hypothesis 依存 inductive + EvolutionStep B4 4-arg post 統合、Section 2.9 残課題解消) + `AgentSpec/Process/HandoffChain.lean` (T1 一時性 inductive、handoff sequence) + Test 2 ファイル **+ Day 6 評価 Section 12.17 / 2.12 から**: 🟡 cross-process interaction test (Hypothesis × Failure × Evolution、Day 4 fullSpineExample パターン踏襲) / 🟡 EvolutionStep B4 4-arg post の I/O type 確定 (Hypothesis/Verdict/Observable 統合) / 🟡 HandoffChain の T1 一時性 inductive 表現 / 🟢 S6 Paper 1 BST/AVL invariants (Hypothesis chain order を invariant 付き structure 化、Section 2.10 paper-grounded) | Week 4-5 Process 層 + Section 2.9 (B4 4-arg post 解消) + Section 2.12 (Day 6 評価導出) + Section 2.10 (S6 Paper 1) |
| **Day 8+** | **Provenance 層着手**: `AgentSpec/Provenance/{ResearchEntity, ResearchActivity, ResearchAgent}.lean` の Lean 化 + mapping 関数 (`Hypothesis.toEntity` / `Failure.toEntity` 等) + 02-data-provenance §4.1-4.4 完全実装の段階的開始 **+ Day 6 評価 Section 12.16 / 2.10 から**: 🟡 02-data-provenance §4.4 退役の構造的検出 (RetiredEntity + Lean linter) / 🟡 Hypothesis rationale Refinement 強化 (Option String → Option Evidence) / 🟡 Failure payload 型化 (各 variant payload を専用型 Evidence/Spec/InconsistencyProof/ResearchEntity に refactor)。または **Manifest 移植** (Week 3-4) または **Lean-Auto research/PoC** (Section 2.10 🔴 対応) を並行開始 | Week 3-4 (Manifest) / Week 4-5 (PROV-O) / Week 6 (Lean-Auto 前倒し) / Section 2.10 + 2.12 (Day 6 評価導出) |
