...
| **Day 17** | Day 16 議論で確定済 Q1-Q4 (Section 2.32): **Q1 A 案** = transitionLegacy 完全削除 A-Standard (breaking change、`since := "2026-04-19"` = Day 17 指定日を履行、段階的 deprecation → removal 完遂) + **Q2 A-Medium** = 定義削除 + test 4 件削除 + `deprecation_history.removal_actual` field 新規 (complete lifecycle 記録 Day 8→16→17) + **Q3 案 A** = Test 先行 update → Production 削除 → build 確認 (intermediate error 回避) + **Q4 案 A** = deprecated 利用 example 4 件全削除 (新 signature 直接展開 proof 2 件は保持、example 13→9): EvolutionStepTest.lean (MODIFY 先行) + EvolutionStep.lean (MODIFY 後続) + artifact-manifest (deprecation_history.removal_actual + week2_status + section_2_15_completely_resolved_day17 + example_count 367→363) + version `0.17.0-week2-day17` + **breaking change** compatibility classification + Day 11-17 = 7 Day 連続 rfl preference 維持記録更新予定 | Section 2.32 (Day 17 着手前判断) + Section 12.46-12.48 (Day 16 評価導出) + Section 2.15 Day 9+ 9 セッション繰り延べ課題完全解消 + 段階的 deprecation → removal 工学的 best practice 完遂 + cycle 内学習 transfer 2 段階別分野転用の 3 Day 完結 (Day 14→16→17) |
