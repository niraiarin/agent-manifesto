...
| 項目 | 現状 | 後続計画 |
|---|---|---|
| `SemVer` 以外の型 | 実装なし、プレースホルダ | Week 2 以降で FolgeID, NodeID, ResearchNode 等を追加 |
| 普遍 round-trip 定理 `∀ v, parse (render v) = some v` | 個別 example + `Fin 5³ = 125` ケース有限量化のみ | Week 2 で induction 証明 (`String.toList` / `Nat.toString` 補題が必要) |
| `Ord SemVer` lexicographic | 実装済、SemVer 専用 | Week 4-5 で `ResearchSpec` Lattice (GA-S15) に拡張 |
| `SemVer` への Hoare-style 4-arg post | 未適用 | Week 4-5 で `ResearchSpec` で適用 (GA-S11, TyDD-B4) |
| `SemVer` の Multiplicity Grading {0,1,ω} | 未適用 | Week 4-5 で検討 (GA-S16, TyDD-F3)、Lean 4 の QTT 非直接対応のため typeclass 模倣が必要 |

### 2.3 `agent-spec-lib/AgentSpec/Test/CoreTest.lean`

| 項目 | 現状 | 後続計画 |
|---|---|---|
| Test/本番ライブラリの混在 | `AgentSpec.lean` が `AgentSpec.Test.CoreTest` を import | Week 2 以降で `lean_lib AgentSpecTest` に分離 (Verifier Round 3 informational 指摘 3) |
| example 8 のコメント不整合 | "DecidableEq" とコメントしているが実際は `rfl` 証明 | 軽微、Week 2 でコメント調整 (Verifier Round 3 informational 指摘 4) |
| Property-based test (Hypothesis 風) | 未実装、`decide` + 有限量化のみ | Week 6-7 で TyDD Recipe 7 適用 (GA-M4 DRT) |

### 2.4 `agent-spec-lib/artifact-manifest.json`

| 項目 | 現状 | 後続計画 |
|---|---|---|
| `$schema` URI | 架空の URL | agent-manifesto 本体 schema 整備時に実在する URL に差し替え |
| JSON スキーマの Lean 型化 (TyDD-J5 Self-hosting preview) | 未実装 — 第 3 回 TyDD 合致度レビューで "微調整候補 (ζ)" として識別 | Week 4 で J5 Self-hosting の一部として Lean `structure ArtifactManifestEntry` 定義。Week 1 内で skeleton 追加も可能だが、JSON ↔ Lean の双方向同期が必要で scope 超過と判断 |
| assumption refs layer | `assumptions: []` 空 | Week 4 で assumption 生成時に充填 (GA-I1 の拡張) |
...
| 項目 | 現状 | 後続計画 |
|---|---|---|
| **Top-down / hole-driven development** (TyDD-S1 benefit #7) | bottom-up で `Core.lean` を一括記述 | Week 2 以降の型定義で `_` placeholder + `#check` ベースの hole-driven スタイルを意識。Lean 4 の hole 機構は限定的だが、段階的型宣言 → 実装の順序を徹底 |
...
| Behavior test | 22-24 件 example + 125 ケース有限量化 | Property-based test (Hypothesis 風) 導入、Week 6-7 |
...
- 指摘 3: テストコードが本番ライブラリ (`AgentSpec`) に直接 import → Week 2 で `lean_lib AgentSpecTest` 分離
- 指摘 4: example 8 (DecidableEq テスト) のコメントと `rfl` 証明手段の不整合
- 指摘 5: `$schema` URL が非解決可能 (`agent-manifesto.internal` ドメイン)
...
**完全合致 12 項目**: S1, S4, D1-D5, F2 (予備), F6, G1-G6, H3, H7, J4, J7, Recipe 11, Recipe 12
...
**更新方針**: Week 2 以降の各 Week 完了時に、完了項目に ~~取り消し線~~ を引き、残タスクの再優先順位を見直す。新規発見タスクは適切な Section に追記。TyDD 合致度レビュー時は Section 12 を更新。
