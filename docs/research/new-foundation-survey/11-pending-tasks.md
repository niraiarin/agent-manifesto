...
| マーク | 項目 | 現状 | 後続計画 |
|---|---|---|---|
| 🔄 Day 2 着手 | Test/本番ライブラリの混在 | `AgentSpec.lean` が `AgentSpec.Test.CoreTest` + **Day 1 追加の `AgentSpec.Test.Spine.FolgeIDTest`** を import | Day 2 で `lean_lib AgentSpecTest` に分離し、`Test/CoreTest.lean` と `Test/Spine/FolgeIDTest.lean` の 2 ファイルを本番 lib から除外 (Verifier Round 3 informational 指摘 3、/verify Round 1 指摘 4 再確認、Day 1 /verify R1 I1 再確認) |
...
| GA-S4 | Edge Type Inductive | Week 2 Day 2 (FolgeID 完成後に参照可) |
...
| **Day 2** | `lean_lib AgentSpecTest` に分離 (lakefile.lean に別 lib target 追加、`Test/CoreTest.lean` **および `Test/Spine/FolgeIDTest.lean`** を移動) / Edge Type (GA-S4) の inductive 型宣言 signature | Section 2.3 🔄 / GA-S4 |
| **Day 3** | Spine 層 `EvolutionStep.lean`, `SafetyConstraint.lean` の type class 宣言 + dummy instance (**Day 1 確立パターン適用**: segment abbrev → structure → Bool helper → instLE 明示命名 → Decidable via `inferInstanceAs`) | Week 2-3 Spine 層 / Section 10.2 |
| **Day 4** | Spine 層 `LearningCycle.lean`, `Observable.lean` の type class 宣言 + dummy instance (**Day 1 確立パターン適用**) | Week 2-3 Spine 層 / Section 10.2 |
| **Day 5** | FolgeID (GA-S2) の `PartialOrder`/`Ord` instance 追加 + behavior example 拡充 / Verifier Round 1 検証 | Week 2-3 Spine 層 + GA-S2 |
