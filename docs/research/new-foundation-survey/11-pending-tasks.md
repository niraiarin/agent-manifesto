...
| **Day 9+** | **Provenance 層継続** (Verdict は Day 8 で先行配置済): `AgentSpec/Provenance/{ResearchEntity, ResearchActivity, ResearchAgent}.lean` + Mapping 関数 (`Hypothesis.toEntity` / `Failure.toEntity` / `Evolution.toActivity` / `Handoff.toAgent`) + 02-data-provenance §4.1-4.7 段階実装 **+ Day 6 評価から**: 🟡 02-data-provenance §4.4 退役の構造的検出 / 🟡 Hypothesis rationale Refinement 強化 / 🟡 Failure payload 型化 **+ Day 7 評価から**: 🟡 S6 Paper 1 BST/AVL invariants (Evolution chain order) / 🟢 Evolution DecidableEq / 🟢 HandoffChain concat **+ Day 8 評価 Section 12.23 / 2.15 から**: 🟡 Verdict payload 拡充 (案 A→C 移行) / 🟢 transitionLegacy deprecated 削除 / 🟢 EvolutionStep 完全 4 member 化 (G5-1 §3.4 step 1) / 🟢 G5-1 §3.4 step 2 LearningM indexed monad。または **Manifest 移植** (Week 3-4) または **Lean-Auto research/PoC** (Section 2.10 🔴) を並行開始 | Week 3-4 (Manifest) / Week 4-5 (PROV-O + LearningM) / Week 6 (Lean-Auto) / Section 2.10 + 2.12 + 2.13 + 2.15 (Day 6-8 評価導出) |
