...
# Test/spec passthrough (defense in depth vs L1 bypass)
# Strict path-component / filename-suffix match (avoids `agent-spec-lib`
# false positive). L1 file guard runs first and owns test-tampering policy;
# this is a belt-and-braces second filter for the lean-cli engagement scope.
if echo "$file_path" | grep -qE '(/[Tt]ests?/|/[Ss]pec/|_test\.|\.test\.|_spec\.|\.spec\.)'; then
  trace "passthrough-test-or-spec-path"
  exit 0
fi
