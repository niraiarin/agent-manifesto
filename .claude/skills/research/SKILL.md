...
### Step 3.1: 事前分解判定（#577 対応）

各 Sub-Issue について以下をチェック:
- [ ] C2: 複数の独立ドメインにまたがらないか？（異なるツールチェイン、異なる知識体系）
- [ ] C3: 精度要求が均一か？（形式証明と探索的設計の混在はないか？）

いずれかに該当する場合、Sub-Issue をさらに分割する。
分割後の各 Sub-Issue にも再帰的に C2, C3 を適用する。

**C2/C3 は judgmental 条件**（`classification_is_judgmental`）。操作的定義の校正は #582, #583 で継続中。
...
2. 先行 Judge で G1-G5 が全て 4.00/5.00 以上
...
### 改善 9: 再帰的 tree 構造と停止基準が未定義 → **暫定統合 (2026-04-16, #577)**

#577 で設計。以下が SKILL.md に統合済み:
- **分解判定関数 v1.0**: 6 条件 (C1-C6)。C1/C4/C6 は計測方法定義済み
...
- **事中分解監視**: Step 5 Checkpoint（C4/C5/C6）
- **統一フロー**: Step 3.1-3.3 + Step 5 Checkpoint + Step 6c.1/6d.1

以下は未完了（#582-584 で継続）:
- C2 (domain_interference) の操作的定義 → #582
- C3 (precision_heterogeneity) の T8 精度スケール → #583
- C5 (multi_hop_depth) の閾値校正 → #584
