...
7. **Archive obligation declaration** (iter 4 empirical 2026-04-30 — iter 1 scenario C で identified: `.injected` ファイル累積 → multi-resume discovery noise): Role enactment section の末尾、または Next Steps の初手として、**「最初に `.claude/handoffs/*.injected` を `.archive/` へ move する」** を明記する。義務 (mandatory)、ベストエフォート不可。これにより次回以降の SessionStart hook が auto-load 候補を decisive に選択でき、recency-heuristic 不要となる。
...
- `.archive/` ディレクトリは無制限に蓄積する。年単位で運用する場合は外部 housekeeping が必要 (例: 90 日以上前の `.archive/handoff-*.injected` を削除 OR 圧縮)。v0.1.0 では housekeeping は人手 + README 注記、自動化は v0.2.0+ candidate。

### 退役 (iter 4 amendment 2026-04-30: best-effort delete → mandatory archive)

- **次セッション (`.injected` を受領 + 再開した側) の初手 task**: `.claude/handoffs/*.injected` を `.claude/handoffs/.archive/` へ move する (`mkdir -p .claude/handoffs/.archive/ && mv <file>.injected .claude/handoffs/.archive/`)。再開完了の前に行ってよい (どうせ二重注入は構造的に防止済)。
  - **削除 (rm) は禁止**: archive 構造 (CONST-006 append-only) を維持するため、`.archive/` 退避が canonical。
  - **mandatory (not best-effort)**: 累積 risk が iter 1 scenario C で identified — `.injected` ファイルが clean up されないと multi-resume 環境で discovery noise になる。
- `.claude/handoffs/handoff-<timestamp>.jsonl` は永続ログとして残る (audit / scripting / 別エージェント統合用、archive しない)。
- 構造的には `.injected` リネームで二重注入を防止済 (D1)。archive はこの不変条件を保ちつつ namespace を clean に保つ補強。
- **Multi-resume discovery natural consequence** (iter 1 scenario C 派生 finding): 上記 archive 規律により `.claude/handoffs/handoff-resume-*.md` (`.injected` 抜き) として残るのは "未消費の active 引き継ぎ" のみ。worktree 並走時に各 branch 1 ファイル、消費されれば即 `.archive/` へ移るので、recency + naming heuristic の非決定性が抑制される。INDEX ファイルや latest pointer は v0.1.0 では追加せず、archive 規律で代替する (シンプル優先、追加 hook 要らない)。
