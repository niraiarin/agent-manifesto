...
description: >
  ...
  単発のバグ修正やリファクタには使わない（直接対応 + /verify で十分）。
...
| 命題 | このスキルとの関係 |
|------|-------------------|
| D2 | Phase 3 で /verify を呼び出し、実装者とは独立したコンテキストで成果物を検証する構造を組み込み |
| D3 | Phase 3 の trace-coverage.sh・manifest-trace を Phase 2 の実装より先に実行可能にし、可観測性基盤を検証フェーズで確立 |
| D5 | Phase 0 で条件付き公理系（仕様）→ Phase 1 でテスト計画 → Phase 2 で実装の順序を強制し、テストが実装に先行 |
