...
| 仮説 | 反証条件 | 現状評価（106回実行データ、Run 106 で更新。observe.sh 自動集計） |
|------|----------|----------------------|
| H1: Agent Teams が学習ライフサイクルの自然なモデル化 | Teams の協調オーバーヘッドが改善効果を上回る | 未反証。107回 success / 2回 partial / 3回 observation。Verifier pass rate: 全期間 359/464 PASS（77%） |
...
| H4: conservative extension 優先が最適戦略 | conservative extension が蓄積し複雑度を増す | 支持傾向。全期間408改善統合（229 conservative extension, 176 compatible change, 1 breaking change, 2 other）。D4 フェーズ順序違反なし |
