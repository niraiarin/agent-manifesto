...
│  │  │ 2+ 件: Judge K=3 ペアワイズ比較 / 1 件: そのまま採用        │
│  │  ▼                                                            │
...
│  │ │   C4: 正確性, C5: 持続性                                   │
│  │ ├─ 各減点を addressable / unaddressable に分類              │
│  │ ├─ addressable 残存 → 修正 → 再判定（最大 2 回）           │
│  │ ├─ addressable 0 件 → PASS（unaddressable は理由記録）     │
...
| Step 1.5: 観察結果永続化 | **deterministic** | JSONL append | 入力は upstream の judgmental だがステップ自体は機械的 |
| Step 2: Hypothesizer | **judgmental** | Hypothesizer agent | 改善案の設計。自動化不可 |
| Step 3: Verifier | **bounded** | Verifier agent | 構造化検証。agent 委譲済み |
| Step 3.5: Judge 評価 | **bounded** | Judge agent | 構造化評価。agent 委譲済み |
...
2. 同一ファイルの同一セクションを変更する提案が 1 件 → そのまま採用
3. 同一グループに 2 件以上の場合 → **Enhanced Dedup** (#557):
   a. Judge agent に K=3 でペアワイズ比較を依頼:
      ```
      Judge: 以下の 2 つの改善提案を C1-C5 基準の総合スコアで比較してください。K=3
      提案 A: [タイトル + 概要]
      提案 B: [タイトル + 概要]
      各提案の C1-C5 総合スコアを算出し、どちらが優れているか判定してください。
      ```
   b. K=3 の平均総合スコアが高い方を採用
   c. 同点 (差 < 0.50) の場合は specificity が高い方を採用（フォールバック）
   d. 3 件以上の場合はトーナメント形式: ペアワイズ比較を繰り返し、各グループから勝者 1 件を残す
4. Dedup で除外された提案は FAIL_LIST に以下の形式で記録:
   ```json
   {"failure_type": "dedup_conflict", "kept": "<採用された提案ID>", "reason": "<採用理由>", "judge_scores": {"kept": X.XX, "removed": [{"id": "<ID>", "score": X.XX}, ...]}}
   ```

**T6 互換**: Enhanced Dedup は resource_revocable=true。人間が Judge の判定を
覆して別の提案を採用することができる。Judge の結果は推奨であり、最終決定ではない。

**フォールバック**: Judge が利用不可能な場合（コスト制約等）は旧方式（specificity ベース）を使用。
...
各 0.25-5.00 点（0.25 刻み, G=20）。**スコアは診断ツールであり、閾値ではない。**
判定基準は「対処可能な減点が残っているか」。

**減点の分類（#289 対応）:**

Judge は各基準のスコアが 5.00 未満の場合、減点を以下に分類する:

| 分類 | 定義 | 例 |
|------|------|-----|
| **addressable** | 現スコープ内で対処可能。修正手順が具体的に示せる | 「docstring に数値を埋め込めば C3 が上がる」 |
| **unaddressable** | 構造的限界。タスクの性質上避けられない | 「メタデータ更新は本質的に C1 が低い」 |
...
Judge の減点フィードバックが事実上捨てられ、呼ぶコストだけ払って改善に使われていなかった。
...
Judge 評価で unaddressable と分類された減点は**必ず行き先を決定する**。
...
判定は deterministic（「この Run の残り項目で対処可能か」の Yes/No）。
