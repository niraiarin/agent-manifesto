...
| verification → judging | "Lean trace: verification -> judging (Phase 3->3.5 in Workflow.lean)" | Step 3: Verifier PASS → Judge 評価 | なし（Run 89 で形式化完了） |
| judging → integration | "Lean trace: judging -> integration (Phase 3.5->4 in Workflow.lean)" | Step 3.5: Judge PASS → Integrator 起動 | なし（Run 89 で形式化完了） |
| judging → hypothesizing (Judge FAIL loopback) | "Lean trace: judging -> hypothesizing (Judge FAIL loopback)" | Step 3.5: Judge FAIL → 再仮説化 | なし（Run 89 で形式化完了） |
...
| `judge_fail_loops_to_hypothesizer` | φ₁₈ | Step 3.5: Judge FAIL → Hypothesizer にループバック | "φ₁₈: judge fail loops to hypothesizer" |
...
| Lean 定理（DesignFoundation.lean） | SKILL.md での運用化 | 備考 |
|-----------------------------------|---------------------|------|
| ~~`d13_coherence_implies_propagation`~~ | _(#251 で退役: trivially-true)_ | #243 参照 |
...
| D10-D14 拡張原則 | `d10_agent_temporary_structure_permanent`, `d10_epoch_monotone`, `d11_structural_minimizes_cost`, `d12_task_is_csp`, `d13_retirement_requires_feedback`, `manifest_has_widest_impact`, `d14_verification_order_is_csp` | DesignFoundation.lean |
| Step 1.5: タスク分類 | `TaskAutomationClass`, `taskMinEnforcement`, `automationPower`, `deterministic_must_be_structural`, `deterministic_minimizes_cost`, `deterministic_judgmental_wasteful`, `observable_implies_automatable`, `automation_enforcement_consistent`, `weak_enforcement_not_deterministic`, `deterministic_strength`, `judgmental_strength`, `classification_exhaustive`, `automation_strict_order` | TaskClassification.lean |

## TaskClassification.lean 逆引きチェックリスト（Run 86 追加）

| Lean 定理/定義（TaskClassification.lean） | SKILL.md での運用化 | 備考 |
|------------------------------------------|---------------------|------|
| `TaskAutomationClass` (inductive) | Phase 2: 実装手段の選択テーブル | deterministic / bounded / judgmental の 3 分類 |
| `taskMinEnforcement` (def) | Phase 2: 分類から強制レイヤーへのマッピング | .deterministic → .structural, .bounded → .procedural, .judgmental → .normative |
| `deterministic_must_be_structural` (theorem) | Phase 2: deterministic タスクは構造的強制 | 「LLM がやるべきでないことを LLM にやらせない」 |
| `deterministic_minimizes_cost` (theorem) | Phase 2: deterministic は全分類中で最小コスト | D11 接続 |
| `deterministic_judgmental_wasteful` (theorem) | Phase 2: 実装手段選択の注意事項 | コスト増大の形式的根拠 |
| `observable_implies_automatable` (theorem) | Phase 2: Observable ならスクリプト化可能 | Observable の運用判定基準 |
| `automation_enforcement_consistent` (theorem) | Phase 2: 分類と強制レイヤーの整合性保証 | 順序構造の一貫性 |
| `automationPower` (def) | Phase 2: 自動化クラスの強度順序（deterministic=3, bounded=2, judgmental=1） | 順序構造の数値マッピング |
| `TaskSuccessCondition` (def) | Phase 2: タスク成功条件（World → Prop） | Observable との接続点 |
| `weak_enforcement_not_deterministic` (theorem) | Phase 2: structural 未満の強制力のタスクは deterministic でない（対偶） | deterministic_must_be_structural の対偶 |
| `deterministic_strength` (theorem) | Phase 2: deterministic の最低強制は structural（strength=3） | 基本性質 |
| `judgmental_strength` (theorem) | Phase 2: judgmental の最低強制は normative（strength=1） | 基本性質 |
| `classification_exhaustive` (theorem) | Phase 2: 3 分類は網羅的（deterministic/bounded/judgmental） | 完全性保証 |
| `automation_strict_order` (theorem) | Phase 2: 分類間の強度は厳密な全順序 | automationPower の単調性 |
