...
# failure_subtype 別の集計（Run 57 以降のエントリ対象。null は旧エントリなので除外）
jq -r '.rejected[]? | select(.failure_subtype != null) | select((.resolved // false) != true) | "\(.failure_type)/\(.failure_subtype): \(.condition // .reason // "no condition")"' .claude/metrics/evolve-history.jsonl 2>/dev/null | sort | uniq -c | sort -rn
```
...
**TaskClassification 推定（任意）:** 各改善候補に対し、タスク特性の推定を付記する。
- `[D]` = deterministic（スクリプトで自動化可能）
- `[B]` = bounded（テスト・証明で検証可能）
- `[J]` = judgmental（LLM・人間の判断が必要）

### 高優先度
1. [観察内容] — [データの根拠]
   **選出根拠**: [何のデータを見て、何と比較し、なぜこの優先度と判断したか]
