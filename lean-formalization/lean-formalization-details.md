...
| `system_health_observable` (Run 27 で theorem に降格) | theorem (was observable-axiom) | 系の健全性 |
...
**問題:** archive/manifesto.md Section 7 の「静止は健全な状態ではない」をどう形式化するか。
...
| 逆サイクル（品質事故→信頼減少→投資縮小） | trust_decreases_on_materialized_risk, risk_reduces_investment | ✅ | risk_reduces_investment（Run 26）で接続済み |
...
| Part III の逆サイクル | ~~信頼減少はあるが投資縮小への接続がない~~ risk_reduces_investment（Run 26）で接続済み | 解消 |
