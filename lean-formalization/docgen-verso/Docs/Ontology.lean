/-
Agent Manifesto - Ontology: Core Type Definitions
...
#doc (Manual) "Ontology: Core Type Definitions" =>
...
*
  * 注入された指示の実行
  * 外部コンテンツに埋め込まれた指示を正当なユーザー指示と区別できず実行
*
  * 信頼境界の侵犯
  * 認証・認可なしに外部システムに作用
*
...
*
  * 不可逆操作の誤実行
  * 悪意ある誘導または判断ミスにより取り消しのできない操作を実行
:::
