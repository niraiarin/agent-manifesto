...
- `references` は List Reference (Day 72 structured 化、DataCite pattern)。
  Day 44 は List String → Day 72 で inductive Reference (doi/url/arxiv/commit) に migration
...
structure Rationale where
  ...
  text : String
  /-- 参照リスト (Day 72: List String → List Reference structured 化、DataCite pattern)。 -/
...
def mk' (text : String) (references : List Reference) (confidence : Nat) : Rationale :=
  { text := text, references := references, confidence := confidence }

/-- reference を append する helper (immutable、新 Rationale 返却)。 -/
def addReference (r : Rationale) (ref : Reference) : Rationale :=
...
def strict (text : String) (references : List Reference) (confidence : Nat)
