...
- **採用**: `strengthenPre_pre_eq_meetPre` / `weakenPost_post_eq_joinPost` を rfl で証明
...
/-! ### Day 78 empirical #6 拡張 (BUG E1 + EDGE E2/E4 修正) -/

/-- ResearchSpec の structure ext lemma (EDGE E4: cases+congr の defensive 強化、
    Day 78+ field 追加に備える)。 -/
theorem ext {S I O : Type} {s1 s2 : ResearchSpec S I O}
    (hpre : s1.pre = s2.pre) (hpost : s1.post = s2.post) : s1 = s2 := by
  cases s1; cases s2; congr

/-- DistribLattice instance (BUG E1: PrePred/PostPred は両方 DistribLattice、
    product も DistribLattice なのに Day 77 では plain Lattice で停止していた)。 -/
instance {S I O : Type} : DistribLattice (ResearchSpec S I O) where
  le_sup_inf _ _ _ := ⟨le_sup_inf, le_sup_inf⟩

/-- OrderTop instance (EDGE E2: trivial が ⊤、`pre := True` / `post := True` が
    Pi/Prop の top と一致)。 -/
instance {S I O : Type} : OrderTop (ResearchSpec S I O) where
  top := { pre := fun _ _ => True, post := fun _ _ _ _ => True }
  le_top _ := ⟨fun _ _ _ => True.intro, fun _ _ _ _ _ => True.intro⟩

/-- OrderBot instance (EDGE E2: unsatisfiable が ⊥)。 -/
instance {S I O : Type} : OrderBot (ResearchSpec S I O) where
  bot := { pre := fun _ _ => False, post := fun _ _ _ _ => False }
  bot_le _ := ⟨fun _ _ h => h.elim, fun _ _ _ _ h => h.elim⟩

/-- ⊤ = trivial (semantic 同値、definitionally)。 -/
theorem top_eq_trivial {S I O : Type} :
    (⊤ : ResearchSpec S I O) = ResearchSpec.trivial := rfl

/-- ⊥ = unsatisfiable (semantic 同値)。 -/
theorem bot_eq_unsatisfiable {S I O : Type} :
    (⊥ : ResearchSpec S I O) = ResearchSpec.unsatisfiable := rfl
